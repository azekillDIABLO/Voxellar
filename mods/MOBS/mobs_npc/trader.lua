...
local mantab = {
	items = {
		...
		{"default:gravel 45", "shop:coin 2", 50},
...
local function mobs_trader(self, clicker, race)
	...
	minetest.chat_send_player(player, S("[NPC] <Trader @1> Hi @2! I have some goods which can interest you! Come and i will show you!.",
...
mobs:register_mob("mobs_npc:trader", {
	...
	jump = false,
	drops = {
		{name = "default:apple", chance = 10, min = 1, max = 4},
		{name = "default:gold_ingot", chance = 25, min = 2, max = 3},
	},
	water_damage = 0,
	lava_damage = 4,
	light_damage = 0,
	...
	on_rightclick = function(self, clicker)
		...
		trader.turn_towards_player(self, clicker)
	end,
})
...
-----------------------------------------------------------------------------------------------------
trader.get_face_direction = function(v1, v2)
        if v1 and v2 then
...
-----------------------------------------------------------------------------------------------------
trader.turn_towards_player = function(self, player)
	if(self.object and self.object.setyaw and self.object.setvelocity) then
		self.object:setvelocity({x=0, y=-5, z=0})
		self.object:setyaw(trader.get_face_direction(self.object:getpos(), player:getpos()));
	end
end

-----------------------------------------------------------------------------------------------------
-- stop moving like crazy
-----------------------------------------------------------------------------------------------------