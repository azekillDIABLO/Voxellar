...
minetest.register_node(":mg_villages:torch_wall", {
	drawtype = "mesh",
	mesh = "torch_wall.obj",
	tiles = {{
		    name = "default_torch_on_floor_animated.png",
...
minetest.register_lbm({
	name = "gamemod:convert_wallmounted",
	nodenames = {"mg_villages:torch", "gamemod:floor", "gamemod:wall"},
	action = function(pos, node)
		if node.param2 >= 2 then
			minetest.set_node(pos, {name = "mg_villages:torch",
				param2 = node.param2})
		else
...
minetest.register_lbm({
	name = "gamemod:convert_wallmounted",
	nodenames = {"default:torch", "gamemod:floor", "gamemod:wall"},
	action = function(pos, node)
		if node.param2 >= 2 then
...
if not minetest.is_yes(minetest.setting_get("gamemod_wieldlight_enable") or true) then
	return
end
local torchlight_update_interval = minetest.setting_get("gamemod_wieldlight_interval") or 0.25

minetest.register_node("gamemod:torchlight", {
	drawtype = "airlike",
...
local function is_torchlight(pos)
	...
	return node.name == "gamemod:torchlight"
end
...
local function place_torchlight(pos)
	...
	if name == "gamemod:torchlight" then
		return true
	end
	...
	if name == "air" then
		minetest.swap_node(pos, {name = "gamemod:torchlight"})
		return true
	end
	return false
end
